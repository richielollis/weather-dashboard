...
searchHistory
  .attr("class", "d-grid m-3 overflow-auto")
  .attr("id", "search-history");
...
var getWeather = function (city) {
  fetch(
    `https://api.openweathermap.org/data/2.5/weather?q=${city}&units=imperial&appid=8a42d43f7d7dc180da5b1e51890e67dc`
  )
    .then(function (res) {
      return res.json();
    })
...
var getFiveDay = function (data) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    fiveDayCard.attr(
      "class",
      "col-sm-12 col-md-2 m-1 card text-bg-primary p-2"
    );
